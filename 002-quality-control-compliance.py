# Case Study: Quality Control and Compliance in Construction Activities
...
Model: AI-based Construction Monitoring

Data Input: Construction logs, quality standards, project plans

Prediction: Quality compliance, construction issues

Recommended Model: AI-based models for construction quality assurance

Customer Value Benefits: Quality Assurance, Cost Savings

Use Case Implementation:
By continuously monitoring construction activities and using AI to detect deviations from quality standards, project managers can address issues promptly, reducing the risk of costly rework and ensuring high-quality outcomes.
"""
...
np.random.seed(42)
construction_logs = np.random.randint(1, 100, size=100)
quality_standards = np.random.randint(1, 100, size=100)
project_plans = np.random.randint(1, 100, size=100)
quality_compliance = np.random.choice([0, 1], size=100)  # 0: Non-compliant, 1: Compliant

# Create DataFrame
data = pd.DataFrame({
    'Construction Logs': construction_logs,
...
data.to_csv('quality_control.csv', index=False)

# Load the data
data = pd.read_csv('quality_control.csv')

# Prepare input (X) and output (y)
X = data[['Construction Logs', 'Quality Standards', 'Project Plans']]
y = data['Quality Compliance']
...
if hasattr(best_model, 'feature_importances_'):
    ...
    feature_names = ['Construction Logs', 'Quality Standards', 'Project Plans']
